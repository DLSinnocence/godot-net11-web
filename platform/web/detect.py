...
def configure(env: "SConsEnvironment"):
    ...
    # Use TempFileMunge since some AR and linker invocations are too long for
    # cmd.exe, especially when statically linking the Mono WASM runtime.
    # Use POSIX-style paths, required with TempFileMunge.
    env["ARCOM_POSIX"] = env["ARCOM"].replace("$TARGET", "$TARGET.posix").replace("$SOURCES", "$SOURCES.posix")
    env["ARCOM"] = "${TEMPFILE('$ARCOM_POSIX','$ARCOMSTR')}"
    env["LINKCOM_POSIX"] = env["LINKCOM"].replace("$TARGET", "$TARGET.posix").replace(
        "$SOURCES", "$SOURCES.posix"
    )
    env["LINKCOM"] = "${TEMPFILE('$LINKCOM_POSIX','$LINKCOMSTR')}"
    # Emscripten's response-file parser treats backslashes as escapes. Use
    # forward slashes for every argument, including library paths generated by
    # SCons' implicit variables.
